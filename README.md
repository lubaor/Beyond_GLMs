...
through `10` for the causal layer.

`fetch_data.py` was last run end to end on 2026-08-28. All six datasets
...
| Step | What it does |
|---|---|
| `01_load_and_profile.py` | Load and profile freMTPL2 |
...
| `07_tuned_gbm_frequency.py` | Nested CV with tuning; writes `07_output.txt` |
...
Only step `07` currently writes a saved log (`07_output.txt`). The other steps
print to stdout; redirect them if you want a record.
...
From `07_output.txt`, 5-fold, tuned GBM against an untuned GLM:
...
under its own upstream terms. Four of the six are GPL (>= 2) and one has no
stated licence: see [DATA_LICENCES.md](DATA_LICENCES.md).
